...
@Composable
fun ShowDetailWeather(modifier: Modifier = Modifier, detailViewModel: DetailViewModel.WeatherDetailState.WeatherData) {
    val weatherCountry = detailViewModel.weather
    val hourlyWeather = detailViewModel.hourlyWeather
    val dailyWeather = detailViewModel.dailyWeatherData
    val city = detailViewModel.city
    val tempUnits=if(detailViewModel.tempUnit=="metric" || detailViewModel.tempUnit=="imperial") "°" else "K"
    val windUnit=if (detailViewModel.windUnit=="m/s") "m/s" else "mph"
    ...
    Column(
        ...
        Text("${city}, ${detailViewModel.country}", color = colorResource(R.color.white), style = MaterialTheme.typography.titleLarge)
